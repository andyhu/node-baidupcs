...
export default class BaiduPCS {
  ...
  wrapApi(func) {
    return async (...params) => {
      ...
      while (retries-- > 0) {
        try {
          while (this.throttle < 1) await Promise.delay(this.options.throttleInterval);
          --this.throttle;
          ...
          ++this.throttle;
          const { retryable, reason } = parseError(err);
  ...
  request = {
    get: async (uri, query = {}) => {
      ...
      const { body } = await req.query({ ...query, access_token: this.options.accessToken })
        .parse(sa.parse['application/json']);
      return body;
    },
    post: async (uri, query = {}, send = {}, attach = []) => {
      ...
      const { body } = await req.parse(sa.parse['application/json']);
      return body;
    },
    download: (uri, query = {}) => {
      const req = sa.get(uri);
