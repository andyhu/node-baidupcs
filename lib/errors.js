const ERRORS = {
  ...
  31066: { status: '404', message: 'file does not exist', explain: '文件不存在', retryable: false },
...
const wrapError = (retryable, reason) => ({ retryable, reason });

export const parseError = err => {
  if (/^ECONNRESET|ECONNREFUSED|ETIMEDOUT|EADDRINFO|ESOCKETTIMEDOUT$/i.test(err.code)) return wrapError(true, err.code);
  if (/^timeout of \d+ms exceeded$/.test(err.message)) return wrapError(true, err.message);
  const res = err.response;
  if (!res) return wrapError(true, err);
  if ([502, 503, 504].indexOf(res.status) !== -1) return wrapError(true, 'Bad gateway');
  if ([408].indexOf(res.status) !== -1) return wrapError(true, 'Request timeout');
  if (res.body && res.body.error_code && ERRORS[res.body.error_code]) {
    return wrapError(ERRORS[res.body.error_code].retryable, res.body);
  }
  return wrapError(true, res.body ? res.body : 'Unknown error');
};
